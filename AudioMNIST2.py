...
from hqa_lightning import HQA as HQA_2Dim
...
if __name__ == '__main__':
    ...
    for i in range(2):
        ...
        logger = TensorBoardLogger("tb_logs", name=f"HQA_AudioMNIST2_Layer{i}")
        trainer = Trainer(max_epochs=EPOCHS,
                          logger=logger,
                          devices=[1],
                          strategy=DDPStrategy(find_unused_parameters=(i > 0))
                          )
        trainer.fit(model=hqa, train_dataloaders=dl)
        hqa_prev = hqa
...
# scipy.io.wavfile.write('test.wav', fs, np.array(audio_signal, dtype=np.int16))
            ...
            plt.yticks([])
            plt.title(str(lab[k]))
        figure1.savefig(f'Ispectr_hat6{i}{k}.png')            
        ...
        for k in range(batch_size):
            ...
            plt.yticks([])
            plt.title(str(lab[k]))
        figure2.savefig(f'Ispectr6{i}{k}.png')
